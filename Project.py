...
class Inspector1:
    def __init__(self):
        ...
        self.sp = np.loadtxt('servinsp1.dat')
        self.number = 0
...
class Workstation1:
    def __init__(self):
        ...
        self.ws = np.loadtxt('ws1.dat')  # number of outputs totally
...
class Inspector2:
    def __init__(self):
        ...
        self.wait_time = 0  # accumulative wait time for inspector1
        self.sp22 = np.loadtxt('servinsp22.dat')
        self.sp23 = np.loadtxt('servinsp23.dat')
        self.number22 = 0
        self.number23 = 0
        self.which = True
...
if __name__ == '__main__':
    ...
    event_list = []  # [time, objective, component, activity]
    event_list += [[World_time + I1.generate(), I1.whichtosend(W1, W2, W3), I1.component(), 'receive']]
    event_list += [[World_time + I2.generate(), I2.whichtosend(W1, W2, W3), I2.component(), 'receive']]
    ...
    while event_list:
        ...
        if event[-1] == 'receive':
            if event[1].buffer[event[2]] <= 1:
                ...
                if generator == I1 and generator.number >= 299:
                    continue
                if generator == I2 and (generator.number22 >= 299 or generator.number23 >= 299):
                    continue
                event_list.append(
                    [World_time + generator.generate(), generator.whichtosend(W1, W2, W3), generator.component(),
                     'receive'])
            else:
                # no room for this component, we have to wait for room to settle down
                k = False
                for eve in event_list:
                    if eve[-1] == 'output' and eve[1] == event[1]:
                        event_list.append([eve[0] + 0.001, eve[1], event[2], 'receive'])
                        k = True
                        break
                if not k and not len(event_list):
